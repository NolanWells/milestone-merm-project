...
export default function Details() {
  ...
  const url = `https://api.themoviedb.org/3/movie/${movie_id}?include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc&with_original_language=en`;
  const options = {
    method: "GET",
    headers: {
    ...
    <Container>
      <CardGroup style={{ border: "solid black 3px" }}>
        ...
        >
          ...
          <Card.Body>
            <details>
              ...
              <b>Release Date:</b> {details && details.release_date}
              <br />
              <b>Runtime:</b> {details && details.runtime} minutes
            </details>
          </Card.Body>
          <br />
        </Card>
        ...
        <Card style={{ border: "solid black 1px" }}>
          ...
          {/*passing down the id so that we can store it with the comment in the database */}
        </Card>
      </CardGroup>
    </Container>
  );
